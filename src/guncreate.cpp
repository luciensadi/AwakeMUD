...
{
  CLS(CH);
  GET_AMMOBOX_INTENDED_QUANTITY(OBJ) = MAX(GET_AMMOBOX_INTENDED_QUANTITY(OBJ), 10);
  ...
  send_to_char(CH, "3) Amount: ^c%d^n  (^c%d^n nuyen)\r\n", GET_AMMOBOX_INTENDED_QUANTITY(OBJ), get_ammo_cost(GET_AMMOBOX_WEAPON(OBJ), GET_AMMOBOX_TYPE(OBJ), GET_AMMOBOX_INTENDED_QUANTITY(OBJ), CH));
