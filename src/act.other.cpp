...
ACMD(do_customize)
{
  ...
  skip_spaces(&argument);
  if (!*argument) {
     send_to_char(ch, "Usage: customize <persona/physical%s%s/background>\r\n", ch, (GET_TRADITION(ch) == TRAD_SHAMANIC || GET_TRADITION(ch) == TRAD_HERMETIC) || (GET_ASPECT(ch) == ASPECT_FULL || GET_ASPECT(ch) == ASPECT_EARTHMAGE || GET_ASPECT(ch) == ASPECT_AIRMAGE || GET_ASPECT(ch) == ASPECT_FIREMAGE || GET_ASPECT(ch) == ASPECT_WATERMAGE) ? "/reflection" : "",
