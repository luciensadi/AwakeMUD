...
ACMD(do_assist)
{
  ...
  if (!*arg)
    ...
    for (opponent = ch->en_room->people; opponent && (FIGHTING(opponent) != helpee);
         opponent = opponent->next_en_room)
      ;
...
bool perform_hit(struct char_data *ch, char *argument, const char *cmdname)
{
  ...
  if (!*arg)
  {
    ...
    return TRUE;
  } else if (!(vict = get_char_room_vis(ch, arg)) && !(veh = get_veh_list(arg, ch->en_room->vehicles, ch)))
  {
    ...
    if (wielded) {
      if (GET_OBJ_TYPE(wielded) == ITEM_FIREWEAPON) {
        ...
        damage_door(ch, ch->en_room, dir, (int)(GET_WEAPON_POWER(wielded) / 2), DAMOBJ_PIERCE);
        return TRUE;
        ...
        damage_door(ch, ch->en_room, dir, (int)(GET_WEAPON_POWER(wielded) / 2), DAMOBJ_PROJECTILE);
        return TRUE;
      } else
      ...
      damage_door(ch, ch->en_room, dir, (int)((GET_STR(ch) + GET_WEAPON_POWER(wielded)) / 2), type);
    } else {
      ...
      damage_door(ch, ch->en_room, dir, (int)(GET_STR(ch) / 2), DAMOBJ_CRUSH);
    }
    return TRUE;
  }
...
ACMD(do_flee)
{
  ...
  for (int tries = 0; tries < 6; tries++) {
    ...
    if (CAN_GO(ch, attempt) && (!IS_NPC(ch) || !ROOM_FLAGGED(&world[ch->en_room->dir_option[attempt]->to_room], ROOM_NOMOB))) {
