...
void check_killer(struct char_data * ch, struct char_data * vict)
{
  ...
  if (!IS_NPC(vict) &&
      ...
      !(ROOM_FLAGGED(ch->en_room, ROOM_ARENA) && ROOM_FLAGGED(vict->en_room, ROOM_ARENA)) &&
      ...
      !PLR_FLAGGED(attacker, PLR_KILLER) && attacker != vict && !IS_SENATOR(attacker))
  {
    ...
    sprintf(buf, "PC Killer bit set on %s for initiating attack on %s at %s.",
            ...
            GET_CHAR_NAME(vict), vict->en_room->name);
  ...
  if (!ch->in_veh) {
    if (ch->followers)
      for (k = ch->followers; k; k = k->next)
        if (PRF_FLAGGED(k->follower, PRF_ASSIST) && k->follower->en_room == ch->en_room && !k->follower->in_veh &&
            !FIGHTING(k->follower) && AWAKE(k->follower))
          do_assist(k->follower, buf, 0, 0);
    if (ch->master)
    {
      if (PRF_FLAGGED(ch->master, PRF_ASSIST) && ch->master->en_room == ch->en_room && !ch->master->in_veh &&
...
void set_fighting(struct char_data * ch, struct veh_data * vict)
{
  ...
  if (ch->followers)
    for (k = ch->followers; k; k = k->next)
      if (PRF_FLAGGED(k->follower, PRF_ASSIST) && k->follower->en_room == ch->en_room &&
          !FIGHTING(k->follower) && AWAKE(k->follower))
        do_assist(k->follower, buf, 0, 0);
  if (ch->master)
  {
    if (PRF_FLAGGED(ch->master, PRF_ASSIST) && ch->master->en_room == ch->en_room &&
        !FIGHTING(ch->master) && AWAKE(ch->master))
      do_assist(ch->master, buf, 0, 0);
    for (k = ch->master->followers; k; k = k->next)
      if (PRF_FLAGGED(k->follower, PRF_ASSIST) && k->follower->en_room == ch->en_room
...
void raw_kill(struct char_data * ch)
{
  ...
  } else
  {
    ...
    if (!IS_NPC(ch)) {
      ...
      else switch (GET_JURISDICTION(ch->en_room)) {
        case ZONE_SEATTLE:
...
void die(struct char_data * ch)
{
  if (!((IS_NPC(ch) && MOB_FLAGGED(ch, MOB_INANIMATE)) || IS_PROJECT(ch) || IS_SPIRIT(ch) || IS_ELEMENTAL(ch))) {
    increase_blood(ch->en_room);
    act("^rBlood splatters everywhere!^n", FALSE, ch, 0, 0, TO_ROOM);
    if (!GET_BACKGROUND_COUNT(ch->en_room) || GET_BACKGROUND_AURA(ch->en_room) == AURA_PLAYERCOMBAT) {
      if (GET_BACKGROUND_AURA(ch->en_room) != AURA_PLAYERDEATH) {
        GET_BACKGROUND_COUNT(ch->en_room) = 1;
        GET_BACKGROUND_AURA(ch->en_room) = AURA_PLAYERDEATH;
      } else {
        GET_BACKGROUND_COUNT(ch->en_room)++;
      }
    }
  }
  if (!IS_NPC(ch))
...
ACMD(do_die)
{
  ...
  sprintf(buf,"%s gave up the will to live. {%s (%ld)}",
          GET_CHAR_NAME(ch),
          ch->en_room->name, ch->en_room->number );
  mudlog(buf, ch, LOG_DEATHLOG, TRUE);
...
void group_gain(struct char_data * ch, struct char_data * victim)
{
  ...
  if (IS_AFFECTED(k, AFF_GROUP) && (k->en_room == ch->en_room))
    tot_members = 1;
  else
    tot_members = 0;
  ...
  for (f = k->followers; f; f = f->next)
    if (IS_AFFECTED(f->follower, AFF_GROUP) && f->follower->en_room == ch->en_room)
...
void dam_message(int dam, struct char_data * ch, struct char_data * victim, int w_type)
{
  ...
  for (witness = victim->en_room->people; witness; witness = witness->next_en_room)
    if (witness != ch && witness != victim && !PRF_FLAGGED(witness, PRF_FIGHTGAG) && SENDOK(witness))
      perform_act(buf, ch, NULL, victim, witness);
  if (ch->en_room != victim->en_room && !PLR_FLAGGED(ch, PLR_REMOTE))
    for (witness = ch->en_room->people; witness; witness = witness->next_en_room)
      if (witness != ch && witness != victim && !PRF_FLAGGED(witness, PRF_FIGHTGAG) && SENDOK(witness))
        perform_act(buf, ch, NULL, victim, witness);
...
int find_orig_dir(struct char_data *ch, struct char_data *victim)
{
  ...
  for (dir = 0; dir < NUM_OF_DIRS; dir++)
  {
    ...
    for (dist = 1; nextroom && dist <= 4; dist++) {
      for (vict = nextroom->people; vict; vict = vict->next_en_room)
        if (vict == ch)
          return (rev_dir[dir]);
...
void weapon_scatter(struct char_data *ch, struct char_data *victim, struct obj_data *weapon)
{
  ...
  for (vict = victim->en_room->people; vict; vict = vict->next_en_room)
  ...
  { // hits a door
    ...
    if (i < 3) {
      ...
      damage_door(NULL, victim->en_room, dir[i], GET_OBJ_VAL(weapon, 0), DAMOBJ_PROJECTILE);
      return;
    }
  }
...
void damage_door(struct char_data *ch, struct room_data *room, int dir, int power, int type)
{
  ...
  rev = rev_dir[dir];
  if (opposite && opposite->dir_option[rev] && &world[opposite->dir_option[rev]->to_room] == room)
    ok = TRUE;
    ...
    case DAMOBJ_EXPLODE:
      ...
      if (GET_OBJ_TYPE(obj) == ITEM_WEAPON && success == 2 &&
        ...
        if (vict) {
          ...
          explode(NULL, obj, obj->en_room);
        } else if (obj->en_room) {
          sprintf(buf, "%s is set off by the flames!",
                  CAP(obj->text.name));
          send_to_room(buf, obj->en_room);
          explode(NULL, obj, obj->en_room);
        }
        return;
      }
...
void docwagon(struct char_data *ch)
{
  ...
  if (success_test(GET_OBJ_VAL(docwagon, 0),
                   MAX(GET_SECURITY_LEVEL(ch->en_room), 4)) > 0)
  {
    ...
    switch (GET_JURISDICTION(ch->en_room)) {
      case ZONE_SEATTLE:
        i = real_room(RM_SEATTLE_DOCWAGON);
        break;
      case ZONE_PORTLAND:
...
#define WRITE_DEATH_MESSAGE(format_string) \
  sprintf(buf2, format_string, GET_CHAR_NAME(vict), GET_ROOM_NAME(vict->en_room), GET_ROOM_VNUM(vict->en_room))
#define WRITE_DEATH_MESSAGE_WITH_AGGRESSOR(format_string) \
  sprintf(buf2, format_string, GET_CHAR_NAME(vict), ch == vict ? "Misadventure(?)" : GET_NAME(ch), GET_ROOM_NAME(vict->en_room), GET_ROOM_VNUM(vict->en_room))
      ...
      switch (number(0, 4)) {
        case 0:
          sprintf(buf2, "%s just hasn't been taking %s medication.  Oops. "
                  ...
                  vict->en_room->name, vict->en_room->number);
          break;
        case 1:
            ...
            bool is_physical)
{
  ...
  if (victim != ch)
  {
    if (GET_POS(ch) > POS_STUNNED && attacktype < TYPE_SUFFERING) {
      ...
      if (!IS_NPC(ch) && IS_NPC(victim) && victim->master && !number(0, 10) &&
          IS_AFFECTED(victim, AFF_CHARM) && (victim->master->en_room == ch->en_room) &&
  ...
  if (ch != victim && !IS_NPC(victim) && !(victim->desc))
  {
    if (!FIGHTING(victim)) {
      ...
      GET_WAS_EN(victim) = victim->en_room;
  ...
  if (attacktype > 0 && ch != victim && attacktype < TYPE_HAND_GRENADE)
  {
    ...
    if (dam > 5) {
      send_to_char(victim, "^RYou slam into the %s at speed, the impact reshaping your body in ways you do not appreciate.^n\r\n",
                   ROOM_FLAGGED(victim->en_room, ROOM_INDOORS) ? "floor" : "ground");
  ...
  if ((ch->en_room != victim->en_room) && total_miss && attacktype < TYPE_SUFFERING &&
...
void astral_fight(struct char_data *ch, struct char_data *vict)
{
      ...
      !PLR_FLAGGED(ch->desc->original, PLR_KILLER))
  {
    ...
    sprintf(buf, "PC Killer bit set on %s (astral) for initiating attack on %s at %s.",
            GET_CHAR_NAME(ch), GET_CHAR_NAME(vict), vict->en_room->name);
    mudlog(buf, ch, LOG_MISCLOG, TRUE);
...
void combat_message_process_ranged_response(struct char_data *ch, rnum_t rnum) {
  for (struct char_data *tch = world[rnum].people; tch; tch = tch->next_en_room) {
    if (IS_NPC(tch)) {
      ...
      if (!IS_NPC(ch) && FIGHTING(ch)
        ...
        if (number(0, 6) >= 2) {
          ...
          struct room_data *was_in = tch->en_room;
          if (ranged_response(ch, tch) && tch->en_room == was_in) {
...
void combat_message(struct char_data *ch, struct char_data *victim, struct obj_data *weapon, int damage, int burst)
{
  ...
  if (ch->en_room == victim->en_room || (ch->in_veh && ch->in_veh->en_room == victim->en_room)) {
    // Same-room messaging.
  ...
  sprintf(been_heard, ".%ld.", real_room(ch->en_room->number));
  ...
  for (int door1 = 0; door1 < NUM_OF_DIRS; door1++) {
    if (ch->en_room->dir_option[door1] && (room1 = ch->en_room->dir_option[door1]->to_room) != NOWHERE && !(world[room1].silence[0])) {
...
bool is_char_too_tall(struct char_data *ch) {
  ...
  return ROOM_FLAGGED(ch->en_room, ROOM_INDOORS) && GET_HEIGHT(ch) >= ch->en_room->z*100;
}
...
void hit(struct char_data *attacker, struct char_data *victim, struct obj_data *weap, struct obj_data *vict_weap)
{
  ...
  if ((melee = !(att->weapon_is_gun))) {
    if (att->ch->en_room != def->ch->en_room) {
  ...
  if (!melee) {
    ...
    if (IS_OBJ_STAT(att->weapon, ITEM_SNIPER) && att->ch->en_room == def->ch->en_room)
    ...
    if (att->ch->en_room != def->ch->en_room && !att->veh) {
      struct char_data *vict;
  ...
  if (att->ch->en_room && !GET_BACKGROUND_COUNT(att->ch->en_room)) {
    GET_BACKGROUND_COUNT(att->ch->en_room) = 1;
    GET_BACKGROUND_AURA(att->ch->en_room) = AURA_PLAYERCOMBAT;
  }
...
int find_sight(struct char_data *ch)
{
  ...
  if (SECT(ch->en_room) != SPIRIT_HEARTH && (IS_NPC(ch) || !access_level(ch, LVL_VICEPRES)))
  ...
  if (!AFF_FLAGGED(vict, AFF_PRONE) && (IS_NPC(vict) && MOB_FLAGGED(vict, MOB_WIMPY) && !MOB_FLAGGED(vict, MOB_SENTINEL))) {
    ...
    for (dir = 0; dir < NUM_OF_DIRS  && !is_responding; dir++) {
      ...
      for (distance = 1; !is_responding && (nextroom && (distance <= 4)); distance++) {
        for (temp = nextroom->people; !is_responding && temp; temp = temp->next_en_room) {
          if (temp == ch && (distance > range || distance > sight) && !(IS_NPC(vict) && MOB_FLAGGED(vict, MOB_SENTINEL))) {
            ...
            char_from_room(vict);
            char_to_room(vict, &world[EXIT2(room, dir)->to_room]);
            if (vict->en_room == ch->en_room) {
    ...
    for (dir = 0; dir < NUM_OF_DIRS && !is_responding; dir++) {
      if (CAN_GO(vict, dir) && &world[EXIT(vict, dir)->to_room] == ch->en_room) {
        is_responding = TRUE;
        act("$n charges towards $s distant foe.", TRUE, vict, 0, 0, TO_ROOM);
        act("You charge after $N.", FALSE, vict, 0, ch, TO_CHAR);
        char_from_room(vict);
        char_to_room(vict, ch->en_room);
        set_fighting(vict, ch);
        act("$n arrives in a rush of fury, immediately attacking $N!", TRUE, vict, 0, ch, TO_NOTVICT);
        act("$n arrives in a rush of fury, rushing straight towards you!", TRUE, vict, 0, ch, TO_VICT);
      }
                  ...
                  int range, int dir)
{
  ...
  if ((ch->char_specials.rigging ? ch->char_specials.rigging->en_room : ch->en_room)->peaceful)
  {
  ...
  if (CAN_GO2(ch->en_room, dir))
    nextroom = &world[EXIT2(ch->en_room, dir)->to_room];
  else
    nextroom = NULL;
  ...
  if (GET_OBJ_TYPE(weapon) == ITEM_WEAPON && GET_OBJ_VAL(weapon, 3) == TYPE_HAND_GRENADE)
  {
    ...
    if (!number(0, 2)) {
      ...
      scatter[0] = ch->en_room;
      if (left > 0 && CAN_GO(ch, left))
        scatter[1] = &world[EXIT(ch, left)->to_room];
      else
        scatter[1] = NULL;
      
      if (right > 0 && CAN_GO(ch, right))
        scatter[2] = &world[EXIT(ch, right)->to_room];
      else
        scatter[2] = NULL;
      
      if (CAN_GO2(nextroom, dir))
        scatter[3] = &world[EXIT2(nextroom, dir)->to_room];
      else
        scatter[3] = NULL;
            ...
            if (GET_OBJ_VAL(weapon, 3) == TYPE_ROCKET)
              for (temp = 0; temp < NUM_OF_DIRS; temp++)
                if (CAN_GO2(scatter[temp2], temp))
                  target_explode(ch, weapon, &world[EXIT2(scatter[temp2], temp)->to_room], 1);
            return;
          }
      }
      target_explode(ch, weapon, vict->en_room, 0);
      if (GET_OBJ_VAL(weapon, 3) == TYPE_ROCKET)
        for (temp = 0; temp < NUM_OF_DIRS; temp++)
          if (CAN_GO2(vict->en_room, temp))
            target_explode(ch, weapon, &world[EXIT2(vict->en_room, temp)->to_room], 1);
    }
    return;
  }
  bool found = FALSE;
      ...
      case WEAP_MISS_LAUNCHER:
        ...
        for (temp = 0; temp < NUM_OF_DIRS; temp++)
          if (CAN_GO2(nextroom, temp))
            target_explode(ch, weapon, &world[EXIT2(nextroom, temp)->to_room], 1);
        break;
      default:
...
void roll_initiative(void)
{
  ...
  for (ch = combat_list; ch; ch = next_combat_list) {
    next_combat_list = ch->next_fighting;
    roll_individual_initiative(ch);
    if (ch->en_room && ch->en_room->poltergeist[0]) {
      int dam = convert_damage(stage(-success_test(GET_QUI(ch), ch->en_room->poltergeist[1] - GET_IMPACT(ch)), LIGHT));
      if (dam > 0) {
...
void perform_violence(void)
{
  ...
  for (ch = combat_list; ch; ch = next_combat_list) {
    ...
    if (!(FIGHTING(ch) || FIGHTING_VEH(ch)) || !AWAKE(ch))
      ...
      if (FIGHTING(ch)) {
        if (IS_NPC(ch) && !ch->desc && GET_SKILL(ch, SKILL_SORCERY) > 0 && GET_MENTAL(ch) > 400 && ch->en_room == FIGHTING(ch)->en_room && success_test(1, 8 - GET_SKILL(ch, SKILL_SORCERY))) {
          mob_magic(ch);
          continue;
        }
        if (ch->squeue) {
      ...
      if (IS_AFFECTED(ch, AFF_APPROACH)) {
        if (IS_AFFECTED(FIGHTING(ch), AFF_APPROACH) || GET_POS(FIGHTING(ch)) < POS_FIGHTING ||
            ...
            ch->en_room != FIGHTING(ch)->en_room) {
          AFF_FLAGS(ch).RemoveBit(AFF_APPROACH);
          ...
          if (footanchor)
            quickness /= 2;
          if (ROOM_FLAGGED(ch->en_room, ROOM_INDOORS) && GET_HEIGHT(ch) > ch->en_room->z*100) {
            if (GET_HEIGHT(ch) > ch->en_room->z * 200)
              quickness = 0;
            else quickness /= 2;
          }
      ...
      if (AFF_FLAGGED(ch, AFF_MANNING) || PLR_FLAGGED(ch, PLR_REMOTE) || AFF_FLAGGED(ch, AFF_RIG)) {
        ...
        RIG_VEH(ch, veh);
        if ((veh && veh->en_room)
            || (FIGHTING(ch) && veh->en_room != FIGHTING(ch)->en_room)
            || (FIGHTING_VEH(ch) && veh->en_room != FIGHTING_VEH(ch)->en_room)) {
        ...
        if (ch->en_room != FIGHTING_VEH(ch)->en_room)
          stop_fighting(ch);
        else
...
void chkdmg(struct veh_data * veh)
{
  if (veh->damage <= 1) {
    ...
    if (veh->towing) {
      veh_to_room(veh->towing, veh->en_room);
      veh->towing = NULL;
    }
...
void vcombat(struct char_data * ch, struct veh_data * veh)
{
  ...
  if (wielded && IS_OBJ_STAT(wielded, ITEM_SNIPER) && ch->en_room == veh->en_room)
    modtarget += 6;
