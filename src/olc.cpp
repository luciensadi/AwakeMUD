  ...
  if (host_num >= 0) {
    ...
    *host = matrix[host_num];
    ...
    if (matrix[host_num].exit) {
      ...
      struct exit_data *temp = host->exit;
      for (struct exit_data *exit_ptr = matrix[host_num].exit; exit_ptr; exit_ptr = exit_ptr->next) {
        ...
        *temp = *exit_ptr;
        ...
        // str_dup any strings
        temp->addresses = (exit_ptr->addresses ? str_dup(exit_ptr->addresses) : NULL);
        temp->roomstring = (exit_ptr->roomstring ? str_dup(exit_ptr->roomstring) : NULL);
