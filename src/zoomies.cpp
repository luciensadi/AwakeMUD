...
bool room_is_valid_flyto_destination(struct room_data *room, struct veh_data *veh, struct char_data *ch) {
  ...
  if (!zone) {
    if (IS_SENATOR(ch))
      send_to_char(ch, "^L(Flight to %s denied: Zone doesn't exist (wat))^n\r\n", GET_ROOM_NAME(room));
    return FALSE;
  }
  ...
  if (zone->is_pghq && !(IS_SENATOR(ch) || (GET_PGROUP_MEMBER_DATA(ch) && GET_PGROUP(ch) && GET_PGROUP(ch)->controls_room(room)))) {
    if (IS_SENATOR(ch))
...
ACMD(do_flyto) {
  ...
  if (GET_NUYEN(ch) < fuel_cost) {
    if (GET_BANK(ch) >= fuel_cost) {
      send_to_char(ch, "You arrange a bank transfer of %d nuyen for fuel, maintenance, and fees, and begin the takeoff process.", fuel_cost);
