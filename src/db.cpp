...
void parse_veh(File &fl, long virtual_nr)
{
  ...
  STR_DUP(veh_proto[veh_nr].name, data.GetString("Name", "unnamed"));
  STR_DUP(veh_proto[veh_nr].short_description, data.GetString("ShortDesc", "An unnamed vehicle"));
  STR_DUP(veh_proto[veh_nr].description, data.GetString("RoomDesc", "An unnamed vehicle idles here."));
  STR_DUP(veh_proto[veh_nr].long_description, data.GetString("LongDesc", "It's a nondescript vehicle."));
  STR_DUP(veh_proto[veh_nr].inside_description, data.GetString("Inside", "You are inside a boring vehicle."));
  STR_DUP(veh_proto[veh_nr].rear_description, data.GetString("InsideRear", "You are inside a boring vehicle."));
  STR_DUP(veh_proto[veh_nr].leave, data.GetString("Leaving", "leaves"));
  STR_DUP(veh_proto[veh_nr].arrive, data.GetString("Arriving", "arrives"));
...
void parse_host(File &fl, long nr)
{
  ...
  STR_DUP(host->name, data.GetString("Name", "An empty host"));
  STR_DUP(host->desc, data.GetString("Description", "This host is empty!\n"));
  STR_DUP(host->keywords, data.GetString("Keywords", "host"));
  STR_DUP(host->shutdown_start, data.GetString("ShutdownStart", "A deep echoing voice announces a host shutdown.\r\n"));
  STR_DUP(host->shutdown_stop, data.GetString("ShutdownStop", "A deep echoing voice announces the shutdown has been aborted.\r\n"));
  ...
  for (int x = 0; x < num_fields; x++) {
    ...
    STR_DUP(exit->number, data.GetString(field, "000"));
    if (host->exit)
      exit->next = host->exit;
    host->exit = exit;
  }
...
void parse_ic(File &fl, long nr)
{
  ...
  STR_DUP(ic->name, data.GetString("Name", "An unfinished IC"));
  STR_DUP(ic->look_desc, data.GetString("LongDesc", "An unfinished IC guards the node."));
  STR_DUP(ic->long_desc, data.GetString("Description", "This IC is unfinished.\r\n"));
...
void parse_room(File &fl, long nr)
{
  ...
  STR_DUP(room->name, data.GetString("Name", "An empty room"));
  STR_DUP(room->description, data.GetString("Desc", "This room is empty!  Boo!\n"));
  STR_DUP(room->night_desc, data.GetString("NightDesc", NULL));
  ...
  STR_DUP(room->address, data.GetString("Address", "An undisclosed location"));
  ...
  for (i = 0; *fulldirs[i] != '\n'; i++) {
    ...
    if (data.DoesSectionExist(sect)) {
      ...
      EQUALS_NEW(room->dir_option[i], room_direction_data);
      ...
      STR_DUP(dir->keyword, data.GetString(field, NULL));
      
      sprintf(field, "%s/Desc", sect);
      STR_DUP(dir->general_description, data.GetString(field, NULL));
  ...
  for (i = 0; true; i++) {
    ...
    if (data.NumFields(sect) > 0) {
      ...
      sprintf(field, "%s/Desc", sect);
      STR_DUP(desc->description, data.GetString(field, NULL));
...
void parse_mobile(File &in, long nr)
{
  ...
  STR_DUP(mob->player.physical_text.keywords, data.GetString("Keywords", "mob unnamed"));
  STR_DUP(mob->player.physical_text.name, data.GetString("Name", "an unnamed mob"));
  STR_DUP(mob->player.physical_text.room_desc, data.GetString("RoomDesc", "An unfinished mob stands here.\n"));
  STR_DUP(mob->player.physical_text.look_desc, data.GetString("LookDesc", "He looks pretty unfinished.\n"));
  
  STR_DUP(mob->char_specials.arrive, data.GetString("ArriveMsg", "arrives from"));
  STR_DUP(mob->char_specials.leave, data.GetString("LeaveMsg", "leaves"));
  GET_TITLE(mob) = NULL;
...
void parse_object(File &fl, long nr)
{
  ...
  STR_DUP(obj->text.keywords, data.GetString("Keywords", "item unnamed"));
  STR_DUP(obj->text.name, data.GetString("Name", "an unnamed item"));
  STR_DUP(obj->text.room_desc, data.GetString("RoomDesc", "An unfinished item is here.\r\n"));
  STR_DUP(obj->text.look_desc, data.GetString("LookDesc", "It looks pretty unfinished.\r\n"));
...
void parse_quest(File &fl, long virtual_nr)
{
  ...
  if (quest_table[quest_nr].num_objs > 0) {
    EQUALS_NEW(quest_table[quest_nr].obj, quest_om_data[quest_table[quest_nr].num_objs]);
  ...
  if (quest_table[quest_nr].num_mobs > 0) {
    EQUALS_NEW(quest_table[quest_nr].mob, quest_om_data[quest_table[quest_nr].num_mobs]);
  ...
  DELETE_ARRAY_IF_EXTANT(quest_table[quest_nr].intro);
  DELETE_ARRAY_IF_EXTANT(quest_table[quest_nr].decline);
  DELETE_ARRAY_IF_EXTANT(quest_table[quest_nr].quit);
  DELETE_ARRAY_IF_EXTANT(quest_table[quest_nr].finish);
  DELETE_ARRAY_IF_EXTANT(quest_table[quest_nr].info);
  DELETE_ARRAY_IF_EXTANT(quest_table[quest_nr].s_string);
  DELETE_ARRAY_IF_EXTANT(quest_table[quest_nr].e_string);
  DELETE_ARRAY_IF_EXTANT(quest_table[quest_nr].done);
...
void parse_shop(File &fl, long virtual_nr)
{
  ...
  STR_DUP(shop->no_such_itemk, data.GetString("NoSuchItemKeeper", "I don't have that."));
  STR_DUP(shop->no_such_itemp, data.GetString("NoSuchItemPlayer", "You don't have that."));
  STR_DUP(shop->not_enough_nuyen, data.GetString("NoNuyen", "You don't have enough nuyen."));
  STR_DUP(shop->doesnt_buy, data.GetString("DoesntBuy", "I don't buy that."));
  STR_DUP(shop->buy, data.GetString("Buy", "That'll be %d nuyen."));
  STR_DUP(shop->sell, data.GetString("Sell", "Here, %d nuyen."));
  STR_DUP(shop->shopname, data.GetString("Name", "Unnamed Shop"));
  ...
  struct shop_sell_data *end = NULL;
  for (int x = 0; x < num_fields; x++) {
...
void load_zones(File &fl)
{
  ...
  if (Z.num_cmds == 0)
    ...
    EQUALS_NEW(Z.cmd, struct reset_com[Z.num_cmds]);
...
void load_saved_veh()
{
  ...
  for (int i = 0; i < num_veh; i++) {
    ...
    STR_DUP(veh->restring, data.GetString("VEHICLE/VRestring", NULL));
    STR_DUP(veh->restring_long, data.GetString("VEHICLE/VRestringLong", NULL));
    ...
    for (int i = 0; i < num_mods; i++) {
      ...
      sprintf(buf, "%s/Name", sect_name);
      STR_DUP(grid->name, data.GetString(buf, NULL));
    ...
    for (int i = 0; i < num_mods; i++) {
      ...
      if (gun && (weapon = read_object(gun, VIRTUAL))) {
        ...
        sprintf(buf, "%s/Name", sect_name);
        STR_DUP(weapon->restring, data.GetString(buf, NULL));
        for (int x = 0; x < NUM_VALUES; x++) {
          sprintf(buf, "%s/Value %d", sect_name, x);
