...
struct program_data programs[] =
  {// Name                     Mult  NERPs  Desc
    {"None",                     0,  FALSE, "" },
    {"Persona (Bod)",            3,  FALSE, "A hardware driver. Required for part installation." },
    {"Persona (Evasion)",        3,  FALSE, "A hardware driver. Required for part installation." },
    {"Persona (Masking)",        2,  FALSE, "A hardware driver. Required for part installation." },
    {"Persona (Sensor)",         2,  FALSE, "A hardware driver. Required for part installation." },
    {"Attack",                   1,  FALSE, "Deals damage to online icons. Cannot damage a decker's physical body or cyberdeck." },
    {"Slow",                     4,  TRUE , "Not implemented, but can be used to attack IC and delay their actions." },
    {"Medic",                    4,  FALSE, "Allows you to patch yourself up." },
    {"Snooper",                  2,  TRUE , "Not implemented, but would reduce the TN for Analyze Operation System Tests. Also would allow spying on a target icon." },
    {"BattleTac",                5,  TRUE , "Not implemented, but would allow two or more Matrix users to link up and gain an initiative bonus (Matrix p72)."},
    {"Compressor",               2,  TRUE , "Not implemented, but would allow you to shrink the size of a file being uploaded." },
    {"Analyze",                  3,  FALSE, "Reduces TNs for System Tests for identifying IC, programs, etc." },
    {"Decrypt",                  1,  FALSE, "Makes it easier for you to defeat Scramble IC by reducing the TN of System Tests against them." },
    {"Deception",                2,  FALSE, "Reduces the TN for most Access Tests." },
    {"Relocate",                 2,  TRUE , "Not implemented, but helps you resist Track utilities used by other deckers." },
    {"Sleaze",                   3,  FALSE, "Makes you harder to detect by augmenting your Masking rating." },
    {"Scanner",                  3,  FALSE, "Reduces the TN of System Tests while trying to locate deckers." },
    {"Browse",                   1,  FALSE, "Reduces the TN for locating specific data values or system addresses-- real-world functions of nodes." },
    {"Read/Write",               2,  FALSE, "Makes it easier to transfer files and perform create/read/update/delete actions on data in the Matrix." },
    {"Track",                    8,  TRUE , "Not implemented, but traces hostile deckers." },
    {"Armor",                    3,  FALSE, "Reduces the power of incoming damage by its rating. Running copy is degraded every time you take damage." },
    {"Camo",                     3,  FALSE, "Makes it harder for trace programs to locate you in meatspace." },
    {"Crash",                    3,  FALSE, "Makes it easier for you to crash the host you're on." },
    {"Defuse",                   2,  FALSE, "Makes it easier to defuse data bombs on paydata etc." },
    {"Evaluate",                 2,  FALSE, "Assists you in locating paydata, your bread and butter in the Matrix." },
    {"Validate",                 4,  FALSE, "Reduces the TN for System Tests related to making admin changes such as accessing system logs and modifying subsystems." },
    {"Swerve",                   3,  FALSE, "Assists you by reducing target numbers in Abort Host Shutdown System Tests." },
    {"Programming Suite",        15, FALSE, "Makes designing and programming easier. Essential for the modern deckbuilder." },
    {"Commlink",                 1,  FALSE, "Reduces TNs for tests that affect your communications link." },
    {"Cloak",                    3,  FALSE, "Makes it easier to evade in Matrix combat." },
    {"Lock-On",                  3,  FALSE, "Reduces the TN for opposed Sensor Tests during combat maneuvers (evading, parrying, repositioning, etc)." },
    {"Cold ASIST Interface",     2,  FALSE, "A hardware driver. Required for part installation." },
    {"Hot ASIST Interface",      4,  FALSE, "A hardware driver. Required for part installation." },
    {"Hardening",                8,  FALSE, "A hardware driver. Required for part installation." },
    {"ICCM Filter",              4,  FALSE, "A hardware driver. Required for part installation." },
    {"Icon Chip",                2,  FALSE, "A hardware driver. Required for part installation." },
    {"MPCP",                     8,  FALSE, "A hardware driver. Required for part installation." },
    ...
    {"Redecorate",               2,  TRUE , "Not implemented, but makes it easier to alter the icon of other Matrix entities." },
    {"Spoof",                    3,  TRUE , "Not implemented, but reduces TNs for affecting system and subsystem slaves." },
    {"Triangulation",            2,  TRUE , "Not implemented, but would let you pinpoint locations of devices such as powered cell phones." },
    {"Counterfeit",              10, TRUE , "Not implemented, and I can't even find it in SR3 or Matrix." },
    {"Guardian",                 2,  TRUE , "Not implemented, but increases the difficulty of an unauthorized user accessing this terminal." },
    {"Remote Control",           3,  TRUE , "Not implemented, but would allow Captain's Chair usage of drones through a separate RCD. Blocks Hacking and Control pool usage while in use." },
    {"Black Hammer",             20, TRUE , "Not implemented, but acts as a black IC program to attack deckers." },
    {"Erosion",                  3,  TRUE , "Not implemented, but attacks a persona rating similar to a Crippler IC." },
    {"Hog",                      3,  TRUE , "Not implemented, but performs a denial-of-service attack against a target cyberterminal." },
    {"Killjoy",                  10, TRUE , "Not implemented, but acts as a non-lethal black IC program." },
...
struct part_data parts[] =
  {
    ...
    { "Active Memory", TYPE_KIT, 0, -1, "Active memory is your RAM; it determines how much you can have running at the same time." },
    { "Storage Memory", TYPE_KIT, 0, -1, "Storage memory is where you keep your programs and downloaded data." },
    { "Hot ASIST Interface", TYPE_KIT, SOFT_ASIST_HOT, 2, "Hot ASIST unlocks your hacking pool, gives +2 Matrix Reaction and +1d6 Matrix Initiative, and lets you use Response Increase hardware." },
    { "Cold ASIST Interface", TYPE_KIT, SOFT_ASIST_COLD, 2, "Cold ASIST protects you from taking lethal damage from black IC, but prevents you from using Response Increase hardware." },
    { "Hardening", TYPE_WORKSHOP, SOFT_HARDENING, 3, "Hardening makes your programs and hardware resilient to damage. Highly recommended." },
    { "ICCM Biofeedback Filter", TYPE_WORKSHOP, SOFT_ICCM, 1, "Increases your chances of jacking out when under attack by black IC, and increases your resistance to black IC and sparky IC damage."},
    { "Icon Chip", TYPE_KIT, SOFT_ICON, 0, "Icon chips control how your persona is projected. In tabletop, swapping out icon chips lets you change what your persona looks like." },
    { "I/O Speed", TYPE_KIT, 0, 0, "Determines how quickly you can upload and download information. More is better." },
    { "MPCP", TYPE_WORKSHOP, SOFT_MPCP, 0, "The heart of your deck, roughly equivalent to a motherboard with CPU. So important that decks are often referred to by their MPCP rating, e.g. 'an MPCP-8 deck'."},
    { "Bod Chip", TYPE_KIT, SOFT_BOD, 0, "Your Bod persona rating (durability), required at a minimum of 1."},
    { "Sensor Chip", TYPE_KIT, SOFT_SENSOR, 0, "Your Sensor (Matrix translation) persona rating, required at a minimum of 1." },
    { "Masking Chip", TYPE_KIT, SOFT_MASKING, 1, "Your Masking (stealth and untraceability) persona rating." },
    { "Evasion Chip", TYPE_KIT, SOFT_EVASION, 1, "Your Evasion (stealth and dodging) persona rating." },
    { "Ports (FUPS)", TYPE_KIT, 0, -1, "Not implemented, but allows connection of external devices including hitching jacks." },
    { "RAS Override", TYPE_KIT, 0, 0, "Suppresses sensory signals from your physical body while decking. Without this, you can perceive the world but take significant penalties." },
