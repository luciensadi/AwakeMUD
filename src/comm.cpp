...
/* act()'s vict_obj is a void *, and perform_act() reads it as whatever the
 * format code asks for: a character for $E $M $N $Q $S, an object for
 * $A $O $P, a plain string for $F $T. A trigger may only be handed it as a
 * character when the message actually names one -- char_script_id() reads
 * and writes through the pointer it is given, so getting this wrong writes
 * into an unrelated object and registers it in the character lookup table. */
static struct char_data *dg_act_victim(const char *str, void *vict_obj)
{
  if (!str || !vict_obj)
    return NULL;
  ...
  for (const char *p = strchr(str, '$'); p; p = strchr(p + 1, '$')) {
    switch (*(p + 1)) {
      case 'E': case 'M': case 'N': case 'Q': case 'S':
        return (struct char_data *) vict_obj;
      case 'A': case 'O': case 'P': case 'F': case 'T':
        return NULL;   /* it is an object or a string, not a character */
    }
  }
...
#define SENDOK(ch) ((ch)->desc && (AWAKE(ch) || sleep) && (!staff_only || IS_SENATOR(ch))  && !(PLR_FLAGGED((ch), PLR_WRITING) || PLR_FLAGGED((ch), PLR_EDITING) || PLR_FLAGGED((ch), PLR_MAILING) || PLR_FLAGGED((ch), PLR_CUSTOMIZE)) && (STATE(ch->desc) != CON_SPELL_CREATE))
  ...
  for (; to; to = next)
  {
    ...
    if (SCRIPT_CHECK(to, MTRIG_ACT))
      act_mtrigger(to, str, ch, dg_act_victim(str, vict_obj), obj, NULL, NULL);
  }
