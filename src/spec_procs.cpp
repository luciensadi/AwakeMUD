...
void attack_random_player(struct char_data *mob, struct char_data *boss)
{
  ...
  for (vict = mob->en_room->people; vict; vict = vict->next_en_room)
...
int summon_mob(struct char_data *ch, int vnum, int number)
{
  ...
  for (tch = character_list; tch && number > 0; tch = tch->next)
    if (GET_MOB_VNUM(tch) == vnum && ch->en_room != tch->en_room &&
...
SPECIAL(janitor)
{
  ...
  for (i = jan->en_room->contents; i; i = i->next_content) {
...
SPECIAL(pike)
{
  ...
  if (CMD_IS("give")) {
    ...
    if (perform_move(ch, WEST, LEADER, NULL) && pike->en_room->number == 2337) {
      if (!IS_SET(EXIT(pike, WEST)->exit_info, EX_CLOSED))
...
SPECIAL(jeff)
{
  ...
  if (CMD_IS("give")) {
    ...
    if (perform_move(ch, EAST, LEADER, NULL) && jeff->en_room->number == 2326) {
      if (!IS_SET(EXIT(jeff, EAST)->exit_info, EX_CLOSED))
...
SPECIAL(gate_guard_park)
{
  ...
  if (CMD_IS("north")) {
    if (perform_move(ch, NORTH, LEADER, NULL) &&
        guard->en_room->number == MANSION_GATE) {
...
SPECIAL(takehero_tsuyama)
{
  ...
  if (!FIGHTING(tsuyama)) {
    for(vict = tsuyama->en_room->people; vict; vict = vict->next_en_room) {
      if (vict != ch && CAN_SEE(tsuyama, vict)
          && ((IS_NPC(vict) || (GET_TKE(vict) > 19 && !IS_SENATOR(vict)))
              && number(0,3) && tsuyama->en_room->number == 4101)) {
...
SPECIAL(saeder_guard)
{
  ...
  if (CMD_IS("east") && CAN_SEE(guard, ch) && guard->en_room->number == 4930) {
...
SPECIAL(crime_mall_guard)
{
  ...
  if ((guard->en_room->number == 10075 && CMD_IS("east")) ||
      (guard->en_room->number == 10077 && CMD_IS("west"))) {
...
SPECIAL(huge_troll)
{
  ...
  if (CMD_IS("west") && troll->en_room->number == 9437 && CAN_SEE(troll, ch)) {
    for (obj = ch->carrying; obj; obj = obj->next_content)
...
SPECIAL(purple_haze_bartender)
{
  ...
  switch (number(0, 18)) {
    case NORTH:
      if (ch->en_room->number == 1844 ||
          ch->en_room->number == 1846) {
...
SPECIAL(yukiya_dahoto)
{
  ...
  if (yukiya != NULL && CMD_IS("open") && CAN_SEE(yukiya, ch) &&
      yukiya->en_room->number == YUKIYA_OFFICE) {
    skip_spaces(&argument);
...
SPECIAL(hand_held_scanner)
{
  ...
  if (ch->en_room->dir_option[dir] &&
      ch->en_room->dir_option[dir]->to_room != NOWHERE) {
    for (i = NORTH; i < NUM_OF_DIRS; i++)
      if (ch->en_room->dir_option[i] &&
          ch->en_room->dir_option[i]->to_room != NOWHERE)
        for (temp = world[ch->en_room->dir_option[i]->to_room].people;
             temp; temp = temp->next_en_room)
          if (temp != ch) {
...
SPECIAL(vendtix)
{
  ...
  if (zone_table[ch->en_room->zone].number == 30)
...
SPECIAL(crime_mall_blockade)
{
  ...
  for (temp = ch->en_room->people; temp && !found; temp = temp->next_en_room)
    if (IS_NPC(temp) && GET_MOB_VNUM(temp) == 10022)
      found = 1;
  if (!found)
    if ((ch->en_room->number == 10075 && CMD_IS("east")) || (ch->en_room->number == 10077 &&
                                                                   CMD_IS("west"))) {
                     ...
                     int display, int factor)
{
  ...
  i = GET_LOADROOM(ch);
  GET_LOADROOM(ch) = ch->en_room->number;
                     ...
                     int cmd, char *arg, int mode)
{
  ...
  save_room = find_hotel_room(ch->en_room->number);
  GET_LOADROOM(ch) = save_room;

  if (mode == RENT_FACTOR)
  {
    act("$n gives you a key and shows you to your room.", FALSE, recep, 0, ch, TO_VICT);
    sprintf(buf, "%s has rented at %ld", GET_CHAR_NAME(ch), ch->en_room->number);
...
SPECIAL(portable_gridguide)
{
  ...
  if (CMD_IS("gridguide") && !ch->in_veh) {
    if (!(ROOM_FLAGGED(ch->en_room, ROOM_ROAD) || ROOM_FLAGGED(ch->en_room, ROOM_GARAGE)) ||
        ROOM_FLAGGED(ch->en_room, ROOM_NOGRID))
      send_to_char(ch, "The %s flashes up with ^RINVALID LOCATION^n.\r\n", GET_OBJ_NAME(obj));
    else {
      int x = ch->en_room->number - (ch->en_room->number * 3), y = ch->en_room->number + 100;
...
SPECIAL(painter)
{
  ...
  vnum_t real_painter_storage = real_room(painter->en_room->number) + 1;
  ...
  if (!(veh = get_veh_list(argument, ch->en_room->vehicles, ch)))
...
SPECIAL(multnomah_gate)
{
  if (!cmd)
    return FALSE;
  long en_room = real_room(get_ch_en_room(ch)->number), to_room = 0;
  if (world[en_room].number == RM_MULTNOMAH_GATE_NORTH)
    to_room = real_room(RM_MULTNOMAH_GATE_SOUTH);
  else to_room = real_room(RM_MULTNOMAH_GATE_NORTH);

  if ((world[en_room].number == RM_MULTNOMAH_GATE_NORTH && CMD_IS("south")) || (world[en_room].number == RM_MULTNOMAH_GATE_SOUTH && CMD_IS("north"))) {
    ...
    if (ch->in_veh) {
      ...
      send_to_room(buf, ch->in_veh->en_room);
      for (struct char_data *vict = ch->in_veh->people; vict; vict = vict->next_in_veh)
...
SPECIAL(locker)
{
  if (!ch || !cmd)
    return FALSE;
  struct obj_data *locker = ch->en_room->contents, *next = NULL;
  int num = 0, free = 0;
  ...
  if (CMD_IS("list")) {
    ...
    if (!free)
      ...
      num = 0;
      for (locker = ch->en_room->contents; locker; locker = locker->next_content)
        if (GET_OBJ_VNUM(locker) == 9826) {
          num++;
          if (!GET_OBJ_VAL(locker, 9))
            break;
