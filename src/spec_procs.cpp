...
void payout_slots(struct obj_data *slots) {
  ...
  if (rolled == 1) {
    ...
    snprintf(buf, sizeof(buf), "There's an electronic fanfare before %s lights up with a brilliant glow, displaying a perfect three-credstick line!\r\n", decapitalize_a_an(GET_OBJ_NAME(slots)));
  } else if (rolled <= 4) {
    // 3 Sevens
    payout_multiplier = 40;
    snprintf(buf, sizeof(buf), "%s dings and whistles loudly before displaying the extremely rare set of three stacks of nuyen!\r\n", CAP(GET_OBJ_NAME(slots)));
  } else if (rolled <= 40) {
    // 3 Cherries
    payout_multiplier = 20;
    snprintf(buf, sizeof(buf), "%s chimes a cheery melody, showing a rare set of three cherries!\r\n", CAP(GET_OBJ_NAME(slots)));
  } else if (rolled <= 580) {
    // 3 of Any Other
    payout_multiplier = 10;
    snprintf(buf, sizeof(buf), "%s flashes celebratory lights as it displays a trio of corporate logos.\r\n", CAP(GET_OBJ_NAME(slots)));
  } else if (rolled <= 1231) {
    // Any 2 Cherries
    payout_multiplier = 3;
    ...
    // Any 1 Cherry
    payout_multiplier = 1;
    snprintf(buf, sizeof(buf), "%s glows happily as it displays a single cherry.\r\n", CAP(GET_OBJ_NAME(slots)));
  } else {
    // No hits
    payout_multiplier = 0;
    snprintf(buf, sizeof(buf), "%s bleeps sympathetically: there's no payout for this pull.\r\n", CAP(GET_OBJ_NAME(slots)));
  }
  ...
  if (CMD_IS("pull") || CMD_IS("use")) {
    ...
    if (*remainder) {
      ...
      if (spend_amount < MINIMUM_SLOT_SPEND_AMOUNT) {
        send_to_char(ch, "The minimum slot spend is %d nuyen.\r\n", MINIMUM_SLOT_SPEND_AMOUNT);
        return TRUE;
      }
      if (spend_amount > MAXIMUM_SLOT_SPEND_AMOUNT) {
        send_to_char(ch, "The maximum slot spend is %d nuyen.\r\n", MAXIMUM_SLOT_SPEND_AMOUNT);
        return TRUE;
      }
    }
