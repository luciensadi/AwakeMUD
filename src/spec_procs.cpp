...
struct obj_data *_find_name_in_tracker(struct obj_data *tracker, const char *name, bool must_be_exact) {
  ...
  for (struct obj_data *obj = tracker->contains; obj; obj = obj->next_content) {
    if (!str_cmp(name, GET_OBJ_NAME(obj)))
      return obj;
  }
  ...
  if (!must_be_exact) {
    for (struct obj_data *obj = tracker->contains; obj; obj = obj->next_content) {
      if (is_abbrev(name, GET_OBJ_NAME(obj)))
        return obj;
    }
  }
...
#define GET_TRACKER_INIT_VALUE(obj) (GET_OBJ_VAL((obj), 0))
#define GET_TRACKER_LOCK_IDNUM(obj) (GET_OBJ_VAL((obj), 1))
void _add_to_initiative_tracker(struct obj_data *tracker, const char *name, int value) {
  ...
  GET_TRACKER_INIT_VALUE(initiative) = value;
  initiative->restring = str_dup(name);
  initiative->in_obj = tracker;

  // Seek for where we can add it.
  if (!(head = tracker->contains) || GET_TRACKER_INIT_VALUE(head) < GET_TRACKER_INIT_VALUE(initiative)) {
  ...
  for (struct obj_data *tmp = head->next_content; tmp; tmp = tmp->next_content) {
    // In-place insert.
    if (GET_TRACKER_INIT_VALUE(tmp) <= GET_TRACKER_INIT_VALUE(initiative)) {
...
SPECIAL(initiative_tracker)
{
  ...
  if (CMD_IS("roll")) {
    if (obj->contains) {
      send_to_char(ch, "You'll have to clear the board with ^WTRACK CLEAR^n first.\r\n");
      return TRUE;
    }

    snprintf(buf2, sizeof(buf2), "%s generates a new initiative track.\r\n", GET_CHAR_NAME(ch));
  ...
  if (CMD_IS("track") || CMD_IS("tracker")) {
    ...
    if (is_abbrev(arg, "lock") || is_abbrev(arg, "unlock")) {
      ...
      if (is_abbrev(arg, "lock")) {
        ...
        if (GET_TRACKER_LOCK_IDNUM(obj)) {
          if (GET_TRACKER_LOCK_IDNUM(obj) != GET_IDNUM(ch)) {
            if (PRF_FLAGGED(ch, PRF_QUESTOR) || access_level(ch, LVL_BUILDER)) {
              snprintf(buf2, sizeof(buf2), "%s overrides and re-locks the initiative tracker.\r\n", GET_CHAR_NAME(ch));
              send_to_room(buf2, get_ch_in_room(ch));
              GET_TRACKER_LOCK_IDNUM(obj) = GET_IDNUM(ch);
            } else {
        ...
        // Add your own lock.
          ...
          GET_TRACKER_LOCK_IDNUM(obj) = GET_IDNUM(ch);
          snprintf(buf2, sizeof(buf2), "%s locks the initiative tracker.\r\n", GET_CHAR_NAME(ch));
          send_to_room(buf2, get_ch_in_room(ch));
          return TRUE;
        }
      }
      ...
      // Attempt to unlock the tracker.
        ...
        if (GET_TRACKER_LOCK_IDNUM(obj)) {
          if (GET_TRACKER_LOCK_IDNUM(obj) != GET_IDNUM(ch)) {
            if (PRF_FLAGGED(ch, PRF_QUESTOR) || access_level(ch, LVL_BUILDER)) {
              snprintf(buf2, sizeof(buf2), "%s overrides and unlocks the initiative tracker.\r\n", GET_CHAR_NAME(ch));
              send_to_room(buf2, get_ch_in_room(ch));
              GET_TRACKER_LOCK_IDNUM(obj) = 0;
            } else {
              send_to_char("The tracker has been locked by someone else.\r\n", ch);
          ...
          // Add your own lock.
            ...
            GET_TRACKER_LOCK_IDNUM(obj) = 0;
            snprintf(buf2, sizeof(buf2), "%s unlocks the initiative tracker.\r\n", GET_CHAR_NAME(ch));
            send_to_room(buf2, get_ch_in_room(ch));
            return TRUE;
          }
        } else {
    ...
    if (is_abbrev(arg, "add")) {
      ...
      _add_to_initiative_tracker(obj, name, new_init_value);
      snprintf(buf2, sizeof(buf2), "%s added %s (%d) to the initiative tracker.\r\n", GET_CHAR_NAME(ch), name, new_init_value);
      send_to_room(buf2, get_ch_in_room(ch));
      return TRUE;
    }
    ...
    if (is_abbrev(arg, "delete")) {
      ...
      snprintf(buf2, sizeof(buf2), "%s removed %s (%d) from the initiative tracker.\r\n", GET_CHAR_NAME(ch), GET_OBJ_NAME(entry_in_tracker), GET_TRACKER_INIT_VALUE(entry_in_tracker));
      send_to_room(buf2, get_ch_in_room(ch));
    ...
    if (is_abbrev(arg, "set")) {
      ...
      snprintf(buf2, sizeof(buf2), "%s updated %s (%d -> %d) in the initiative tracker.\r\n", GET_CHAR_NAME(ch), GET_OBJ_NAME(entry_in_tracker), GET_TRACKER_INIT_VALUE(entry_in_tracker), new_init_value);
      send_to_room(buf2, get_ch_in_room(ch));
      GET_TRACKER_INIT_VALUE(entry_in_tracker) = new_init_value;
      return TRUE;
    }

    if (is_abbrev(arg, "list")) {
      if (!obj->contains) {
        send_to_char("There isn't an initiative list at the moment. Use 'roll initiative' to create one for the whole room, or add individuals with ^WTRACK ADD <name> <value>^n.\r\n", ch);
        return TRUE;
      }

      send_to_char("Current initiative list:\r\n", ch);
      for (struct obj_data *temp = obj->contains; temp; temp = temp->next_content) {
        send_to_char(ch, "%d) %s\r\n", GET_TRACKER_INIT_VALUE(temp), GET_OBJ_NAME(temp));
      }
      return TRUE;
    ...
    if (is_abbrev(arg, "advance")) {
      ...
      if (!obj->contains) {
        send_to_char(ch, "There's nothing to advance. Start a new list with ^WROLL INITIATIVE^n.\r\n");
        return TRUE;
      }

      snprintf(buf2, sizeof(buf2), "%s advanced the initiative list to the next pass.\r\n", GET_CHAR_NAME(ch));

      struct obj_data *next;
      for (struct obj_data *temp = obj->contains; temp; temp = next) {
        next = temp->next_content;
        if ((GET_TRACKER_INIT_VALUE(temp) -= 10) <= 0) {
          snprintf(ENDOF(buf2), sizeof(buf2) - strlen(buf2), " - %s is out of initiative.\r\n", GET_OBJ_NAME(temp));
          obj_from_obj(temp);
          extract_obj(temp);
        }
      }
