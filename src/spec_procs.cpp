    ...
    for (struct obj_data *soft = obj->contains; soft; soft = soft->next_content) {
      ...
      if (GET_OBJ_TYPE(soft) == ITEM_DESIGN) {
        ...
        if (GET_DESIGN_PROGRAMMING_TICKS_LEFT(soft)) {
          completion_percentage = ((float) 100) - ((float) GET_DESIGN_PROGRAMMING_TICKS_LEFT(soft) / MAX(1, GET_DESIGN_ORIGINAL_TICKS_LEFT(soft)) * 100);
          ...
          completion_percentage = ((float) 100) - ((float) GET_DESIGN_DESIGNING_TICKS_LEFT(soft) / MAX(1, GET_DESIGN_ORIGINAL_TICKS_LEFT(soft)) * 100);
